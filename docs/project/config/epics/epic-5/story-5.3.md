...
## Acceptance Criteria
- [x] `POST /api/calls/{call_id}/artifacts` accepts a list of `{artifact_type_id, mode}` selections
  - Creates artifact rows: `mode = 'claude'` → `status = 'pending'`; `mode = 'manual'` → `status = 'done'` with empty content
- [x] `GET /api/calls/{call_id}/artifacts/stream` opens an SSE stream
...
- [x] Each artifact row's `prompt_used` is set to the artifact type's current prompt at generation time (immutable snapshot)
- [x] Claude model used: `claude-sonnet-4-6`
- [x] Rate limit (429): retry with exponential backoff (max 3 retries), then mark artifact as `error`
- [x] One artifact error does not block the others
- [x] All Claude calls logged: start, token count on success, error on failure
- [x] NFR-08: no generation starts until `POST /api/calls/{id}/artifacts` is explicitly called

## Tasks
- [x] Create `backend/services/claude_service.py` — `generate_artifact(artifact_id, prompt, transcript) → str`
- [x] Implement retry logic in `claude_service.py` (3 retries, exponential backoff on 429)
- [x] Create `backend/routers/artifacts.py` — POST to create selections, GET `/stream` SSE endpoint
- [x] `POST` creates artifact rows with `prompt_used` snapshot
- [x] `GET /stream` uses `asyncio.as_completed` for parallel generation
- [x] Register router in `backend/main.py`
- [x] Write tests: `backend/tests/test_artifacts.py` (mock Anthropic client)
